...
function populateClothesShopMenu(clothesTypeArray, businessName, priceMultiplier) {
	if(messagesLoaded) {
		...
		// Wait for the messages to be loaded
		clearTimeout(timeout);
		timeout = setTimeout(function() { populateClothesShopMenu(clothesTypeArray, businessName, priceMultiplier); }, 100);
	}
}
...
function populateWardrobeMenu(clothesTypeArray) {
	if(messagesLoaded) {
		...
		// Wait for the messages to be loaded
		clearTimeout(timeout);
		timeout = setTimeout(function() { populateWardrobeMenu(clothesTypeArray); }, 100);
	}
}
